...
export const Button: React.FC<ButtonProps> = ({ 
  ...
  const variants = {
    ...
    wipe: "relative overflow-hidden bg-transparent border border-brand-oxford/10 dark:border-white/10 text-brand-oxford dark:text-white rounded-xl p-1.5 pr-6 transition-all duration-300 hover:border-brand-azure dark:hover:border-brand-robin group",

    // Wipe White: Ghost outline that wipes fill (for colored backgrounds like Brand Azure)
    "wipe-white": "relative overflow-hidden bg-transparent border border-white/30 text-white rounded-xl p-1.5 pr-6 transition-all duration-300 hover:border-white group",
  };
  ...
  if (variant === 'wipe' || variant === 'wipe-white') {
    ...
    const wipeBg = isWhite ? 'bg-white' : 'bg-brand-azure';
    const iconBg = isWhite ? 'bg-white text-brand-azure' : 'bg-brand-azure text-white';
    const hoverText = isWhite ? 'group-hover:text-brand-azure' : 'group-hover:text-white';
    ...
    return (
      ...
      >
        {/* Wipe Background */}
        <span className={`absolute left-0 top-0 bottom-0 w-12 rounded-lg ${wipeBg} transition-all duration-500 ease-[cubic-bezier(0.76,0,0.24,1)] group-hover:w-full`}></span>
  ...
  return (
    <button 
      ...
      {...props}
    >
      <span className={variant === 'primary' ? 'mr-3' : ''}>{children}</span>
      {IconComponent && (
        <span className={`
          ${variant === 'circle' ? '' : ''}
          ${variant === 'primary' ? 'w-6 h-6 rounded-md bg-brand-azure/10 dark:bg-brand-robin/10 text-brand-azure dark:text-brand-robin flex items-center justify-center group-hover:bg-white group-hover:text-brand-azure transition-colors' : 'ml-2'}
        `}>
          <IconComponent size={variant === 'circle' ? 20 : 16} />
        </span>
      )}
    </button>
  );
